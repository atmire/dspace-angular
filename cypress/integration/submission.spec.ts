...
describe('New Submission page', () => {
  ...
  it('should create a new submission when using /submit path & pass accessibility', () => {
    ...
    cy.visit(
      '/submit?collection=' + TEST_SUBMIT_COLLECTION_UUID + '&entityType=none'
    );
    ...
    cy.get('#collectionControlsMenuButton span').should(
      'have.text',
      TEST_SUBMIT_COLLECTION_NAME
    );
  ...
  it('should block submission & show errors if required fields are missing', () => {
    ...
    cy.get('div#traditionalpageone-header i.fa-exclamation-circle').should(
      'be.visible'
    );
    ...
    cy.location().then((fullUrl) => {
      ...
      cy.intercept('GET', '/server/api/discover/search/objects*').as(
        'search-results'
      );
  ...
  it('should allow for deposit if all required fields completed & file uploaded', () => {
    ...
    cy.intercept('POST', '/server/api/submission/workspaceitems/*').as(
      'upload'
    );

    // Upload our DSpace logo via drag & drop onto submission form
    // cy.get('div#section_upload')
    cy.get('div.ds-document-drop-zone').selectFile(
