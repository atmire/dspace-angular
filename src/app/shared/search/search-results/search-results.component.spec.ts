...
describe('SearchResultsComponent', () => {
  ...
  beforeEach(waitForAsync(() => {
    TestBed.configureTestingModule({
      ...
      declarations: [SearchResultsComponent, QueryParamsDirectiveStub],
  ...
  it('should display results when results are not empty', () => {
    (comp as any).searchResults = {
      hasSucceeded: true,
      isLoading: false,
      payload: { page: { length: 2 } },
    };
  ...
  it('should display link with new search where query is quoted if search return a error 400', () => {
    ...
    const linkDes = fixture.debugElement.queryAll(
      By.directive(QueryParamsDirectiveStub)
    );

    // get attached link directive instances
    // using each DebugElement's injector
    const routerLinkQuery = linkDes.map((de) =>
      de.injector.get(QueryParamsDirectiveStub)
    );

    expect(routerLinkQuery.length).toBe(
      1,
      'should have 1 router link with query params'
    );
    expect(routerLinkQuery[0].queryParams.query).toBe(
      '"foobar"',
      'query params should be "foobar"'
    );
  });
  ...
  it('should not add quotes around the given string if they are already there', () => {
    expect(comp.surroundStringWithQuotes('"teststring"')).toEqual(
      '"teststring"'
    );
  });
