...
export class ComcolMetadataComponent<TDomain extends Community | Collection>
  implements OnInit
{
  /**
  ...
  onSubmit(event) {
    ...
    if (newLogo) {
      this.dsoDataService
        .getLogoEndpoint(event.dso.uuid)
        .pipe(take(1))
    ...
    if (!isEmpty(event.operations)) {
      this.dsoDataService
        .patch(event.dso, event.operations)
        ...
        .subscribe(async (response: RemoteData<DSpaceObject>) => {
          if (response.hasSucceeded) {
            ...
            this.notificationsService.success(
              null,
              this.translate.get(
                `${this.type.value}.edit.notifications.success`
              )
            );
            ...
            this.notificationsService.error(
              null,
              this.translate.get(
                `${this.type.value}.edit.notifications.unauthorized`
              )
            );
          } else {
            this.notificationsService.error(
  ...
  navigateToHomePage() {
    this.dsoRD$
      .pipe(getFirstSucceededRemoteData(), take(1))
