...
export class RelationGroupFieldParser extends FieldParser {
  public modelFactory(
    ...
    const modelConfiguration: DynamicRelationGroupModelConfig = this.initModel(
      null,
      label
    );
    ...
    if (
      ...
      this.configData.rows.forEach((row: FormRowModel) => {
        row.fields.forEach((field: FormFieldModel) => {
          if (
            field.selectableMetadata[0].metadata ===
            ...
            modelConfiguration.mandatoryField =
              this.configData.selectableMetadata[0].metadata;
          } else {
            modelConfiguration.relationFields.push(
              field.selectableMetadata[0].metadata
            );
          }
        });
    ...
    if (isNotEmpty(this.getInitGroupValues())) {
      ...
      const mandatoryFieldEntries: FormFieldMetadataValueObject[] =
        this.getInitFieldValues(modelConfiguration.mandatoryField);
      mandatoryFieldEntries.forEach((entry, index) => {
        const item = Object.create(null);
        const listFields = [modelConfiguration.mandatoryField].concat(
          modelConfiguration.relationFields
        );
        listFields.forEach((fieldId) => {
          const value = this.getInitFieldValue(0, index, [fieldId]);
          item[fieldId] = isNotEmpty(value)
            ? value
