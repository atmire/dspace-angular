...
export class RelationshipEffects {
   ...
   */
  mapLastActions$ = createEffect(
    () =>
      this.actions$.pipe(
        ofType(
          RelationshipActionTypes.ADD_RELATIONSHIP,
          RelationshipActionTypes.REMOVE_RELATIONSHIP
        ),
        map((action: RelationshipAction) => {
          const { item1, item2, submissionId, relationshipType } =
            action.payload;
          const identifier: string = this.createIdentifier(
            item1,
            item2,
            relationshipType
          );
          if (hasNoValue(this.debounceMap[identifier])) {
            ...
            this.debounceMap[identifier] = new BehaviorSubject<string>(
              action.type
            );
            this.debounceMap[identifier]
              .pipe(this.debounceTime(DEBOUNCE_TIME), take(1))
              .subscribe((type) => {
                if (this.initialActionMap[identifier] === type) {
                  if (type === RelationshipActionTypes.ADD_RELATIONSHIP) {
                    let nameVariant = (action as AddRelationshipAction).payload
                      .nameVariant;
   ...
   */
  updateNameVariantsActions$ = createEffect(
    () =>
      this.actions$.pipe(
        ofType(RelationshipActionTypes.UPDATE_NAME_VARIANT),
        map((action: UpdateRelationshipNameVariantAction) => {
          const { item1, item2, relationshipType, submissionId, nameVariant } =
            action.payload;
          const identifier: string = this.createIdentifier(
            item1,
            item2,
          ...
          if (inProgress) {
            ...
            this.relationshipService
              .updateNameVariant(item1, item2, relationshipType, nameVariant)
              .pipe(
                filter((relationshipRD: RemoteData<Relationship>) =>
                  hasValue(relationshipRD.payload)
                ),
                take(1)
              )
              .subscribe((c) => {
   ...
   */
  updateRelationshipActions$ = createEffect(
    () =>
      this.actions$.pipe(
   ...
   */
  saveSubmissionSection = createEffect(() =>
    this.actions$.pipe(
      ofType(
        ServerSyncBufferActionTypes.EMPTY,
        JsonPatchOperationsActionTypes.COMMIT_JSON_PATCH_OPERATIONS
      ),
      filter(() => hasValue(this.updateAfterPatchSubmissionId)),
      switchMap(() =>
        this.refreshWorkspaceItemInCache(this.updateAfterPatchSubmissionId)
      ),
      map(
        (submissionObject) =>
          new SaveSubmissionSectionFormSuccessAction(
            this.updateAfterPatchSubmissionId,
            [submissionObject],
            false
          )
      )
  ...
  constructor(
    ...
    @Inject(DEBOUNCE_TIME_OPERATOR)
    private debounceTime: <T>(
      dueTime: number
  ...
  private addRelationship(
    ...
    return this.relationshipTypeService
      .getRelationshipTypeByLabelAndTypes(relationshipType, type1, type2)
      .pipe(
        ...
        switchMap((rd: RemoteData<Relationship>) => {
          if (hasNoValue(rd) || rd.hasFailed) {
            ...
            this.selectableListService
              .findSelectedByCondition(
                listId,
                (object: any) =>
                  hasValue(object.indexableObject) &&
                  object.indexableObject.uuid === item2.uuid
              )
              .pipe(take(1), hasValueOperator())
              .subscribe((selected) => {
                this.selectableListService.deselectSingle(listId, selected);
              });
            let errorContent;
            if (hasNoValue(rd)) {
              errorContent = this.translateService.instant(
            ...
            this.notificationsService.error(
              this.translateService.instant('relationships.add.error.title'),
              errorContent
            );
          }
  ...
  private removeRelationship(
    ...
    this.relationshipService
      .getRelationshipByItemsAndLabel(item1, item2, relationshipType)
      .pipe(
        mergeMap((relationship: Relationship) =>
          this.relationshipService.deleteRelationship(relationship.id, 'none')
        ),
        take(1),
   ...
   */
  private refreshWorkspaceItemInCache(
    submissionId: string
  ): Observable<SubmissionObject> {
    return this.submissionObjectService
      .findById(submissionId, false, false, followLink('item'))
