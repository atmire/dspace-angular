...
export class EditRelationshipListComponent implements OnInit, OnDestroy {
   ...
   */
  relationshipsRd$: BehaviorSubject<RemoteData<PaginatedList<Relationship>>> =
    ...
    return observableCombineLatest(
      ...
      map(([label, relatedEntityType]) => {
        if (
          ...
          const relationshipLabel = `${label.substring(
            2,
            label.indexOf('Of')
          )}`;
  ...
  private getLabel(): Observable<string> {
    return observableCombineLatest(
      [this.relationshipType.leftType, this.relationshipType.rightType].map(
        (itemTypeRD) =>
          itemTypeRD.pipe(getFirstSucceededRemoteData(), getRemoteDataPayload())
      )
    ).pipe(
      map(
        (itemTypes: ItemType[]) =>
  ...
  openLookup() {
    ...
    const modalComp: DsDynamicLookupRelationModalComponent =
    ...
    modalComp.select = (...selectableObjects: SearchResult<Item>[]) => {
      selectableObjects.forEach((searchResult) => {
        ...
        const foundIndex = modalComp.toRemove.findIndex(
          (el) => el.uuid === relatedItem.uuid
        );

        if (foundIndex !== -1) {
          modalComp.toRemove.splice(foundIndex, 1);
        } else {
          this.getRelationFromId(relatedItem).subscribe(
            (relationship: Relationship) => {
              if (!relationship) {
                modalComp.toAdd.push(searchResult);
              } else {
                const foundIndexRemove = modalComp.toRemove.findIndex(
                  (el) => el.indexableObject.uuid === relatedItem.uuid
                );
    ...
    modalComp.deselect = (...selectableObjects: SearchResult<Item>[]) => {
      selectableObjects.forEach((searchResult) => {
        ...
        const foundIndex = modalComp.toAdd.findIndex(
          (el) => el.indexableObject.uuid === relatedItem.uuid
        );
    ...
    modalComp.submitEv = () => {
      ...
      modalComp.toAdd.forEach((searchResult: SearchResult<Item>) => {
        ...
        subscriptions.push(
          this.relationshipService
            .getNameVariant(this.listId, relatedItem.uuid)
            .pipe(
              map((nameVariant) => {
                const update = {
                  uuid:
                    ...
                    '-' +
                    searchResult.indexableObject.uuid,
                  nameVariant,
      ...
      modalComp.toRemove.forEach((searchResult) => {
        subscriptions.push(
          this.relationshipService
            .getNameVariant(this.listId, searchResult.indexableObjectuuid)
            .pipe(
              switchMap((nameVariant) => {
                return this.getRelationFromId(
                  searchResult.indexableObject
                ).pipe(
    ...
    this.relatedEntityType$.pipe(take(1)).subscribe((relatedEntityType) => {
      modalComp.relationshipOptions = Object.assign(new RelationshipOptions(), {
  ...
  getRelationFromId(relatedItem) {
    return this.currentItemIsLeftItem$.pipe(
      ...
      switchMap((isLeft) => {
        let apiCall;
        if (isLeft) {
          apiCall = this.relationshipService
   ...
   */
  private getFieldUpdatesForRelatedItem(
    relatedItem: Item
  ): Observable<RelationshipIdentifiable[]> {
      ...
      mergeMap((identifiables) =>
        observableCombineLatest(
          identifiables.map((identifiable) =>
            this.getRelatedItem(identifiable.relationship)
          )
        ).pipe(
          defaultIfEmpty([]),
          map((relatedItems) => {
  ...
  private getIsRelatedItem(relatedItem: Item): Observable<boolean> {
    return this.currentItemIsLeftItem$.pipe(
      ...
      map((isLeft) => {
        if (isLeft) {
          const listOfRelatedItems = this.item.allMetadataValues(
            'relation.' + this.relationshipType.leftwardType
          );
          return !!listOfRelatedItems.find((uuid) => uuid === relatedItem.uuid);
        } else {
          const listOfRelatedItems = this.item.allMetadataValues(
            'relation.' + this.relationshipType.rightwardType
          );
          return !!listOfRelatedItems.find((uuid) => uuid === relatedItem.uuid);
        }
      })
  ...
  ngOnInit(): void {
    ...
    this.relationshipLeftAndRightType$ = observableCombineLatest(
      [this.relationshipType.leftType, this.relationshipType.rightType].map(
        (type) =>
          type.pipe(getFirstSucceededRemoteData(), getRemoteDataPayload())
      )
    ) as Observable<[ItemType, ItemType]>;

    this.relatedEntityType$ = this.relationshipLeftAndRightType$.pipe(
      map((relatedTypes: ItemType[]) =>
        relatedTypes.find(
          (relatedType) => relatedType.uuid !== this.itemType.uuid
        )
      ),
      hasValueOperator()
    );
    ...
    this.relatedEntityType$
      ...
      .subscribe(
        (relatedEntityType) =>
          (this.listId = `edit-relationship-${this.itemType.id}-${relatedEntityType.id}`)
      );
    ...
    this.currentItemIsLeftItem$ = this.relationshipLeftAndRightType$.pipe(
      map(([leftType, rightType]: [ItemType, ItemType]) => {
        ...
        // should never happen...
        console.warn(
          `The item ${this.item.uuid} is not on the right or the left side of relationship type ${this.relationshipType.uuid}`
        );
        return undefined;
      })
    );
    ...
    const currentPagination$ = this.paginationService
      .getCurrentPagination(this.paginationConfig.id, this.paginationConfig)
      .pipe(tap(() => this.loading$.next(true)));

    this.subs.push(
      observableCombineLatest([currentPagination$, this.currentItemIsLeftItem$])
        .pipe(
          switchMap(
            ([currentPagination, currentItemIsLeftItem]: [
    ...
    this.subs.push(
      this.relationshipsRd$
        .pipe(hasValueOperator(), getAllSucceededRemoteData())
        .subscribe((rd: RemoteData<PaginatedList<Relationship>>) => {
      ...
      this.relationshipsRd$
        .pipe(
          ...
          switchMap((rd: RemoteData<PaginatedList<Relationship>>) =>
            ...
            observableFrom(rd.payload.page).pipe(
              mergeMap((relationship: Relationship) =>
                ...
                this.relationshipService
                  .isLeftItem(relationship, this.item)
                  .pipe(
              ...
              map(([relationship, isLeftItem]: [Relationship, boolean]) => {
                ...
                const nameVariant = isLeftItem
                  ? relationship.rightwardValue
                  : relationship.leftwardValue;
                return {
                  uuid: relationship.id,
                  type: this.relationshipType,
                  relationship,
          ...
          switchMap((nextFields: RelationshipIdentifiable[]) => {
            ...
            return this.objectUpdatesService
              .getFieldUpdates(this.url, nextFields)
              .pipe(
                map((fieldUpdates: FieldUpdates) => {
                  ...
                  Object.keys(fieldUpdates).forEach((uuid) => {
                    if (hasValue(fieldUpdates[uuid])) {
                      const field = fieldUpdates[uuid]
                        .field as RelationshipIdentifiable;
                      ...
                      if (field.type.id === this.relationshipType.id) {
                        ...
                        if (
                          fieldUpdates[uuid].changeType === FieldChangeType.ADD
                        ) {
                          // increase the counter that tracks new relationships
                          this.nbAddedFields$.next(
                            this.nbAddedFields$.getValue() + 1
                          );
