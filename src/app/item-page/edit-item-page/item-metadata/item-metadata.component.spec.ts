...
describe('ItemMetadataComponent', () => {
  beforeEach(waitForAsync(() => {
    ...
    paginatedMetadataFields = createPaginatedList([
      mdField1,
      mdField2,
      mdField3,
    ]);
    ...
    metadataFieldService = jasmine.createSpyObj({
      getAllMetadataFields: createSuccessfulRemoteDataObject$(
    ...
    objectUpdatesService = jasmine.createSpyObj('objectUpdatesService', {
      ...
      createPatch: observableOf([operation1]),
    });
  ...
  describe('submit', () => {
    ...
    it('it should call reinstateFieldUpdates on the objectUpdatesService with the correct url and metadata', () => {
      ...
      expect(objectUpdatesService.getFieldUpdates).toHaveBeenCalledWith(
        url,
        comp.item.metadataAsList
      );
    });
  });
  ...
  describe('hasChanges', () => {
    describe("when the objectUpdatesService's hasUpdated method returns true", () => {
      ...
      it('should return an observable that emits true', () => {
        const expected = '(a|)';
        scheduler
          .expectObservable(comp.hasChanges())
