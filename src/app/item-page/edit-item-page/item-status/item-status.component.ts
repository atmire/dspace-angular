    ...
    this.itemRD$
      ...
      */
        ...
        operations.push(
          new ItemOperation(
            'authorizations',
            this.getCurrentUrl(item) + '/authorizations',
        ...
        operations.push(
          new ItemOperation(
            'mappedCollections',
            this.getCurrentUrl(item) + '/mapper',
        ...
        if (item.isWithdrawn) {
          operations.push(
            new ItemOperation(
              'reinstate',
              this.getCurrentUrl(item) + '/reinstate',
          ...
          operations.push(
            new ItemOperation(
              'withdraw',
              this.getCurrentUrl(item) + '/withdraw',
        ...
        if (item.isDiscoverable) {
          operations.push(
            new ItemOperation(
              'private',
              this.getCurrentUrl(item) + '/private',
          ...
          operations.push(
            new ItemOperation(
              'public',
              this.getCurrentUrl(item) + '/public',
              FeatureID.CanMakePrivate,
              true
            )
          );
        ...
        operations.push(
          new ItemOperation(
            'delete',
            this.getCurrentUrl(item) + '/delete',
            FeatureID.CanDelete,
            true
          )
        );
        ...
        observableFrom(operations)
          .pipe(
            mergeMap((operation) => {
              if (hasValue(operation.featureID)) {
                return this.authorizationService
                  .isAuthorized(operation.featureID, item.self)
                  .pipe(
                    distinctUntilChanged(),
                    map(
                      (authorized) =>
