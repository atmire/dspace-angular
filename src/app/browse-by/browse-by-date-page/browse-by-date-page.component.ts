...
export class BrowseByDatePageComponent extends BrowseByMetadataPageComponent {
      ...
      observableCombineLatest([
        ...
        .pipe(
          map(([routeParams, queryParams, data, currentPage, currentSort]) => {
            return [
              Object.assign({}, routeParams, queryParams, data),
              currentPage,
              currentSort,
            ];
          })
        ...
        .subscribe(
          ([params, currentPage, currentSort]: [
            ...
            const metadataKeys = params.browseDefinition
              ? params.browseDefinition.metadataKeys
              : this.defaultMetadataKeys;
            this.browseId = params.id || this.defaultBrowseId;
            this.startsWith = +params.startsWith || params.startsWith;
            const searchOptions = browseParamsToOptions(
              params,
              currentPage,
              currentSort,
              this.browseId
  ...
  updateStartsWithOptions(
    ...
    this.subs.push(
      this.browseService
        .getFirstItemFor(definition, scope)
        .subscribe((firstItemRD: RemoteData<Item>) => {
          ...
          if (hasValue(firstItemRD.payload)) {
            ...
            if (isNotEmpty(date) && isValidDate(date)) {
              ...
              lowerLimit = isNaN(dateObj.getUTCFullYear())
                ? lowerLimit
          ...
          const oneYearBreak =
            Math.floor((currentYear - environment.browseBy.oneYearLimit) / 5) *
            5;
          const fiveYearBreak =
            Math.floor(
              (currentYear - environment.browseBy.fiveYearLimit) / 10
            ) * 10;
