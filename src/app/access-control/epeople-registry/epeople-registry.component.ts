...
export class EPeopleRegistryComponent implements OnInit, OnDestroy {
   ...
   */
  ePeople$: BehaviorSubject<PaginatedList<EPerson>> = new BehaviorSubject(
    buildPaginatedList<EPerson>(new PageInfo(), [])
  );
  /**
   * A BehaviorSubject with the list of EpersonDtoModel objects made from the EPeople in the repository or
   * as the result of the search
   */
  ePeopleDto$: BehaviorSubject<PaginatedList<EpersonDtoModel>> =
    new BehaviorSubject<PaginatedList<EpersonDtoModel>>({} as any);
  ...
  search(data: any) {
    ...
    this.findListOptionsSub = this.paginationService
        ...
        switchMap((findListOptions) => {
          ...
          if (query != null && this.currentSearchQuery !== query) {
            this.router.navigate(
              [this.epersonService.getEPeoplePageRouterLink()],
              {
          ...
          return this.epersonService.searchByScope(
          ...
          if (confirm) {
            if (hasValue(ePerson.id)) {
              this.epersonService
                ...
                .subscribe((restResponse: RemoteData<NoContent>) => {
                  if (restResponse.hasSucceeded) {
                    ...
                    this.notificationsService.error(
                      'Error occured when trying to delete EPerson with id: ' +
                        ePerson.id +
                        ' with code: ' +
