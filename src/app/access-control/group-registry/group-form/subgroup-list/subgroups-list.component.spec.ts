...
describe('SubgroupsListComponent', () => {
  ...
  it('should create SubgroupsListComponent', inject(
    [SubgroupsListComponent],
    (comp: SubgroupsListComponent) => {
  ...
  it('should show list of subgroups of current active group', () => {
    const groupIdsFound = fixture.debugElement.queryAll(
      By.css('#subgroupsOfGroup tr td:first-child')
    );
  ...
  describe('if first group delete button is pressed', () => {
    ...
    beforeEach(fakeAsync(() => {
      const addButton = fixture.debugElement.query(
        By.css('#subgroupsOfGroup tbody .deleteButton')
      );
    ...
    it('one less subgroup in list from 1 to 0 (of 2 total groups)', () => {
      groupsFound = fixture.debugElement.queryAll(
        By.css('#subgroupsOfGroup tbody tr')
      );
  ...
  describe('search', () => {
    describe('when searching with empty query', () => {
      ...
      it('should display all groups', () => {
        ...
        const groupIdsFound = fixture.debugElement.queryAll(
          By.css('#groupsSearch tbody tr td:first-child')
        );
          ...
          if (getSubgroups !== undefined && getSubgroups.length > 0) {
            groupsFound.map((foundGroupRowElement) => {
              if (foundGroupRowElement.debugElement !== undefined) {
                const addButton = foundGroupRowElement.debugElement.query(
                  By.css('td:last-child .fa-plus')
                );
                const deleteButton = foundGroupRowElement.debugElement.query(
            ...
            getSubgroups.map((group: Group) => {
              groupsFound.map((foundGroupRowElement) => {
                if (foundGroupRowElement.debugElement !== undefined) {
                  const groupId = foundGroupRowElement.debugElement.query(
