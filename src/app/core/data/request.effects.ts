...
export class RequestEffects {
  execute = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap((action: RequestExecuteAction) => {
        return this.requestService.getByUUID(action.payload).pipe(take(1));
      }),
      ...
      mergeMap((request: RestRequest) => {
        ...
        if (isNotEmpty(request.body) && !request.isMultipart) {
          const serializer = new DSpaceSerializer(
            getClassForType(request.body.type)
          );
        ...
        return this.restApi
          ...
          .pipe(
            map((data: RawRestResponse) =>
              this.injector
                .get(request.getResponseParser())
                .parse(request, data)
            ),
            map(
   ...
   */
  fixTimestampsOnRehydrate = createEffect(() =>
    this.actions$.pipe(
