...
export class LookupRelationService {
  ...
  ): Observable<RemoteData<PaginatedList<SearchResult<Item>>>> {
    const newConfig = Object.assign(
      new PaginatedSearchOptions({}),
      searchOptions,
      {
        fixedFilter: relationship.filter,
        configuration: relationship.searchConfiguration,
      }
    );
    if (setSearchConfig) {
    ...
    return this.searchService.search(newConfig).pipe(
      ...
      multicast(
        ...
        (subject) =>
          subject.pipe(
            takeWhile(
              (rd: RemoteData<PaginatedList<SearchResult<DSpaceObject>>>) =>
                rd.isLoading
            ),
  ...
  getTotalLocalResults(
    ...
    return this.getLocalResults(
      ...
      getRemoteDataPayload(),
      map(
        (results: PaginatedList<SearchResult<Item>>) => results.totalElements
      ),
      startWith(0)
    );
  }
  ...
  getTotalExternalResults(
    ...
    return this.externalSourceService
      ...
      .pipe(
        ...
        getRemoteDataPayload(),
        map(
          (results: PaginatedList<ExternalSourceEntry>) => results.totalElements
        ),
        startWith(0),
        distinctUntilChanged()
      );
