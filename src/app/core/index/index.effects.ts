...
export class UUIDIndexEffects {
  addObject$ = createEffect(() =>
    this.actions$.pipe(
      ...
      filter((action: AddToObjectCacheAction) =>
        hasValue(action.payload.objectToCache.uuid)
      ),
   ...
   */
  addAlternativeObjectLink$ = createEffect(() =>
    this.actions$.pipe(
      ofType(ObjectCacheActionTypes.ADD),
      map((action: AddToObjectCacheAction) => {
    ...
    this.actions$.pipe(
      ...
      filter(
        (action: RequestConfigureAction) =>
          action.payload.method === RestRequestMethod.GET
      ),
