...
export class CollectionSourceControlsComponent implements OnDestroy {
  ...
  ngOnInit() {
    ...
    this.contentSource$ = this.collectionService
      .findByHref(this.collection._links.self.href, false)
      .pipe(
        getAllSucceededRemoteDataPayload(),
        switchMap((collection) =>
          this.collectionService.getContentSource(collection.uuid, false)
        ),
        getAllSucceededRemoteDataPayload()
      );
  }
    ...
    this.subs.push(
      this.scriptDataService
        ...
        .pipe(
          ...
          switchMap((rd) =>
            this.processDataService.findById(rd.payload.processId, false)
          ),
          getAllCompletedRemoteData(),
        ...
        .subscribe((process: Process) => {
          ...
          if (
            ...
            this.bitstreamService
              .findByHref(process._links.output.href)
              .pipe(getFirstSucceededRemoteDataPayload())
              .subscribe((bitstream) => {
                this.httpClient
                  .get(bitstream._links.content.href, { responseType: 'text' })
                  .subscribe((data: any) => {
                    const output = data
        ...
        .subscribe((process) => {
          if (
            ...
            setTimeout(() => {
              ...
              this.requestService.setStaleByHrefSubstring(
                this.collection._links.self.href
              );
            }, 5000);
          }
          if (
