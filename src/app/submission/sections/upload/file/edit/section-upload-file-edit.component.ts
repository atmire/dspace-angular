...
export class SubmissionSectionUploadFileEditComponent implements OnInit {
  ...
  public setOptions(model: DynamicFormControlModel, control: FormControl) {
    ...
    if (isNotEmpty(accessCondition)) {
      const showGroups: boolean =
        accessCondition.hasStartDate === true ||
        accessCondition.hasEndDate === true;

      const startDateControl: FormControl = control.parent.get(
        'startDate'
      ) as FormControl;
      const endDateControl: FormControl = control.parent.get(
        'endDate'
      ) as FormControl;
  ...
  protected buildFileEditForm() {
    ...
    if (this.collectionPolicyType === POLICY_DEFAULT_WITH_LIST) {
      ...
      const confStart = {
        relations: [
          { match: MATCH_ENABLED, operator: OR_OPERATOR, when: hasStart },
        ],
      };
      const confEnd = {
        relations: [
          { match: MATCH_ENABLED, operator: OR_OPERATOR, when: hasEnd },
        ],
      };
