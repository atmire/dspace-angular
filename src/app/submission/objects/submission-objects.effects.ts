...
export class SubmissionObjectEffects {
    ...
    this.actions$.pipe(
      ...
      map((action: InitSubmissionFormAction) => {
        ...
        definition.sections.page.forEach((sectionDefinition: any) => {
          const selfLink =
            sectionDefinition._links.self.href || sectionDefinition._links.self;
          const sectionId = selfLink.substr(selfLink.lastIndexOf('/') + 1);
          const config = sectionDefinition._links.config
            ? sectionDefinition._links.config.href ||
              sectionDefinition._links.config
            : '';
          const enabled =
          ...
          if (sectionDefinition.sectionType !== SectionsType.SubmissionForm) {
            sectionData =
              isNotUndefined(action.payload.sections) &&
              isNotUndefined(action.payload.sections[sectionId])
                ? action.payload.sections[sectionId]
                : Object.create(null);
          } else {
            sectionData = action.payload.item.metadata;
          }
          const sectionErrors = isNotEmpty(action.payload.errors)
            ? action.payload.errors[sectionId] || null
            : null;
          mappedActions.push(
      ...
      mergeMap((result) => {
        return observableFrom(
          result.mappedActions.concat(
            new CompleteInitSubmissionFormAction(
  ...
  resetForm$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(
        (action: ResetSubmissionFormAction) =>
          ...
          .pipe(
            map(
              (response: SubmissionObject[]) =>
                new SaveSubmissionFormSuccessAction(
                  action.payload.submissionId,
                  response,
                  action.payload.isManual
                )
            ),
            catchError(() =>
              observableOf(
                new SaveSubmissionFormErrorAction(action.payload.submissionId)
              )
            )
          );
      })
   ...
   */
  saveForLaterSubmission$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((action: SaveForLaterSubmissionFormAction) => {
        return this.operationsService
          ...
          .pipe(
            map(
              (response: SubmissionObject[]) =>
                new SaveForLaterSubmissionFormSuccessAction(
                  action.payload.submissionId,
                  response
                )
            ),
   ...
   */
  saveSubmissionSuccess$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map(([action, currentState]: [SaveSubmissionFormSuccessAction, any]) => {
        return this.parseSaveResponse(
          (currentState.submission as SubmissionState).objects[
            action.payload.submissionId
          ],
   ...
   */
  saveSubmissionSectionSuccess$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((action: SaveSubmissionSectionFormAction) => {
        return this.operationsService
          ...
          .pipe(
            map(
              (response: SubmissionObject[]) =>
                new SaveSubmissionSectionFormSuccessAction(
                  action.payload.submissionId,
                  response
                )
            ),
            catchError(() =>
              observableOf(
                new SaveSubmissionSectionFormErrorAction(
                  action.payload.submissionId
                )
              )
            )
      ...
      this.actions$.pipe(
        ...
        tap(() =>
          this.notificationsService.error(
            null,
            this.translate.get('submission.sections.general.save_error_notice')
          )
        )
      ),
    { dispatch: false }
      ...
      switchMap(
        ([action, hasUnsavedModification]: [
          ...
          if (hasUnsavedModification) {
            ...
            response$ = this.submissionObjectService
              .findById(action.payload.submissionId, false, true)
              .pipe(
          ...
          return response$.pipe(
            map((response: SubmissionObject[]) => {
              if (this.canDeposit(response)) {
                ...
                return new SaveSubmissionFormSuccessAction(
                  action.payload.submissionId,
                  response
                );
              }
   ...
   */
  depositSubmission$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([action, state]: [DepositSubmissionAction, any]) => {
        return this.submissionService
          .depositSubmission(
            state.submission.objects[action.payload.submissionId].selfUrl
          )
          .pipe(
            map(
              () =>
                new DepositSubmissionSuccessAction(action.payload.submissionId)
            ),
            catchError((error) =>
              observableOf(
                new DepositSubmissionErrorAction(action.payload.submissionId)
              )
            )
          );
      })
   ...
   */
  saveForLaterSubmissionSuccess$ = createEffect(
    () =>
      this.actions$.pipe(
        ofType(
          SubmissionObjectActionTypes.SAVE_FOR_LATER_SUBMISSION_FORM_SUCCESS
        ),
        tap(() =>
   ...
   * Show a notification on success and redirect to MyDSpace page
   */
  depositSubmissionSuccess$ = createEffect(
    () =>
      this.actions$.pipe(
   ...
   * Show a notification on error
   */
  depositSubmissionError$ = createEffect(
    () =>
      this.actions$.pipe(
   ...
   */
  discardSubmission$ = createEffect(() =>
    this.actions$.pipe(
      ofType(SubmissionObjectActionTypes.DISCARD_SUBMISSION),
      switchMap((action: DepositSubmissionAction) => {
        return this.submissionService
          .discardSubmission(action.payload.submissionId)
          .pipe(
            map(
              () =>
                new DiscardSubmissionSuccessAction(action.payload.submissionId)
            ),
            catchError(() =>
              observableOf(
                new DiscardSubmissionErrorAction(action.payload.submissionId)
              )
            )
          );
      })
   ...
   */
  addAllMetadataToSectionData = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((action: UpdateSectionDataAction) => {
        return this.sectionService
          ...
          .pipe(
            map((section: SubmissionSectionObject) => [action, section]),
            take(1)
          );
      }),
      filter(
        ([action, section]: [
          UpdateSectionDataAction,
          SubmissionSectionObject
        ]) => section.sectionType === SectionsType.SubmissionForm
      ),
      switchMap(
        ([action, section]: [
          UpdateSectionDataAction,
              ...
              map(
                (metadata: any) =>
                  new UpdateSectionDataAction(
                    ...
                    action.payload.sectionId,
                    metadata,
                    action.payload.errorsToShow,
                    action.payload.serverValidationErrors,
   ...
   * Show a notification on success and redirect to MyDSpace page
   */
  discardSubmissionSuccess$ = createEffect(
    () =>
      this.actions$.pipe(
   ...
   * Show a notification on error
   */
  discardSubmissionError$ = createEffect(
    () =>
      this.actions$.pipe(
  ...
  protected parseSaveResponse(
    ...
    if (isNotEmpty(response)) {
      if (notify) {
        this.notificationsService.success(
          null,
          this.translate.get('submission.sections.general.save_success_notice')
        );
      }
      ...
      response.forEach((item: WorkspaceItem | WorkflowItem) => {
        ...
        const sections: WorkspaceitemSectionsObject =
          item.sections && isNotEmpty(item.sections) ? item.sections : {};
        const sectionsKeys: string[] = union(
        ...
        for (const sectionId of sectionsKeys) {
          ...
          if (
            currentState.sections[sectionId].sectionType ===
              SectionsType.Upload &&
          ...
          if (notify && !currentState.sections[sectionId].enabled) {
            this.submissionService.notifyNewSection(
              submissionId,
              sectionId,
              currentState.sections[sectionId].sectionType
            );
