...
export class GoogleRecaptchaService {
  ...
  public getRecaptchaToken(action) {
    return this.captchaKey().pipe(
      switchMap((key) => this._window.nativeWindow.grecaptcha.execute(key, { action: action })),
    );
  }
  ...
  public executeRecaptcha() {
    return of(this._window.nativeWindow.grecaptcha.execute());
  }

  public getRecaptchaTokenResponse() {
    return this._window.nativeWindow.grecaptcha.getResponse();
  }
